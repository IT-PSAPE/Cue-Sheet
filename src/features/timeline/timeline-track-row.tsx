...
import { TRACK_COLORS } from '@/utils/cue-sheet/cue-sheet-utils'
import { TRACK_HEIGHT } from '@/utils/timeline/timeline-constants'
...
export function TimelineTrackRow({ track, index, isEditing, editingTrackName, canDeleteTracks, isDragSource, isDragTarget, onTrackDragStart, onEditingTrackNameChange, onUpdateTrackName, onCancelRename, onRenameTrack, onToggleTrackHidden, onToggleTrackLocked, onDeleteTrack, onSetTrackColor }: TimelineTrackRowProps) {
  const rowClassName = `relative flex items-center gap-2 border-b border-[var(--border-secondary,#e8e8e8)] px-[var(--spacing-md,8px)] pb-px transition-colors ${track.hidden ? 'opacity-45' : ''} ${isDragSource ? 'bg-pink-50' : ''}`
