...
import { getCueTypeIcon } from '@/utils/cues/cue-type-icons'
import type { CueItem } from '@/types/cue-sheet'
import { isTrackLocked } from '@/utils/cue-sheet/cue-sheet-utils'
import { TRACK_HEIGHT, TIME_RULER_HEIGHT, TIMELINE_HORIZONTAL_PADDING } from '@/utils/timeline/timeline-constants'
import { formatTimeDisplay, getMarkerInterval, buildTimeMarkers, getCuesByTrack, getTrackColor, getCueTypeById } from '@/utils/timeline/timeline-utils'
  ...
  return (
    <ScrollArea className="min-h-0 relative flex-1 overflow-auto touch-auto overscroll-contain" containerRef={onTimelineContainerRef} onScroll={handleTimelineScroll}>
      <div style={{ width: timelineWidth, minWidth: '100%' }} ref={trackRowsRef} className="relative min-h-full">
        ...
        <button type="button" onPointerDown={handlePlayheadPointerDown} aria-label="Drag playhead" className="absolute top-0 z-30 h-10 w-10 -translate-x-1/2 flex items-center justify-center pb-2 text-pink-500 touch-none" style={{ left: playheadLeft }}>
