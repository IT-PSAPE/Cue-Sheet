import type { CueItemFormData, CueType, Event, EventFormData, TrackFormData } from '@/types/cue-sheet'
import { createCueItem, createDefaultCueTypes, createEvent, createTrack } from '@/utils/cue-sheet/cue-sheet-utils'
import { mergeCueTypes } from '@/utils/cue-sheet/cue-sheet-transfer'
...
export type CueSheetAction =
  ...
  | { type: 'IMPORT_DATA'; payload: { events: Event[]; cueTypes?: CueType[] } }
...
export function cueSheetReducer(state: CueSheetState, action: CueSheetAction): CueSheetState {
  switch (action.type) {
    ...
    case 'DELETE_EVENT': {
      if (state.events.length <= 1) return state
    ...
    case 'IMPORT_DATA': {
      const importedCueTypes = action.payload.cueTypes ?? []
      const cueTypes = importedCueTypes.length > 0 ? mergeCueTypes(state.cueTypes, importedCueTypes) : state.cueTypes
      const validTypeIds = new Set(cueTypes.map((cueType) => cueType.id))
      const fallbackTypeId = cueTypes[0]?.id ?? createDefaultCueTypes()[0].id
      const importedEvents = action.payload.events.map((event) => ({
        ...event,
        cueItems: event.cueItems.map((cueItem) => ({
          ...cueItem,
          type: validTypeIds.has(cueItem.type) ? cueItem.type : fallbackTypeId,
      ...
      const events = [...state.events, ...importedEvents]
      return {
        ...state,
        events,
        cueTypes,
        selectedEventId: state.selectedEventId ?? importedEvents[0]?.id ?? null,
      }
    }
    default:
      return state
