...
import type { CueItemFormData, CueType, EventFormData } from '@/types/cue-sheet'
import { useCueSheet } from './use-cue-sheet'
import { useCueDialogState, type AddCueDefaults } from '../cues/use-cue-dialog-state'
...
export function useCueSheetScreen(): UseCueSheetScreenValue {
  ...
  const canDeleteEvents = state.events.length > 1
  ...
  const handleDeleteEvent = useCallback(() => {
    ...
    closeDeleteEvent()
  }, [canDeleteEvents, closeDeleteEvent, dispatch, resetCueDialogs, selectedEvent])
